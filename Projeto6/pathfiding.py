...
class FourWay(GetStrategy):
    def get_neighbors(self, grid, node):
        ...
        for nr, nc in neighbors:
            if 0 <= nr < grid.shape[0] and 0 <= nc < grid.shape[1] and grid[nr, nc] is None:
...
class EightWay(GetStrategy):
    def get_neighbors(self, grid, node):
        ...
        for dr, dc in [(1, 1), (1, -1), (-1, 1), (-1, -1)]:
            nr, nc = r + dr, c + dc
            if 0 <= nr < grid.shape[0] and 0 <= nc < grid.shape[1] and grid[nr, nc] is None:
                if grid[r, nc] is None and grid[nr, c] is None:
...
class AStarPathfinder(Pathfinder):
    ...
    def find_path(self, grid, start, goal):
        ...
        if grid[start] is not None or grid[goal] is not None:
            return []
